            ...
            <table class="min-w-full text-sm bg-white divide-y-2 divide-gray-200">
                <thead class="ltr:text-left rtl:text-right">
                    <tr>                        
                        ...
                        @for($i=1; $i <= $audioCounts; $i++)
                            <th class="px-4 py-2 text-gray-900 whitespace-nowrap">audio {{$i}}</th>
                        @endfor
                ...
                <tbody class="divide-y divide-gray-200">
                    @foreach ($pronunciations as $pronunciation)
                        <tr class="hover:bg-gray-50">
                            ...
                            @for ($i = 1; $i <= $audioCounts; $i++)
                                @if(!empty($pronunciation->audio[$i]))
                                    <td class="px-6 py-4">
                                        <x-admin.audio :getAudio="$pronunciation->getAudio($i)"/>
                                    </td>
                                @else 
