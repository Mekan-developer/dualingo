<div class="w-full mt-4" wire:ignore>
    <div class="relative w-full text-blue-900 hover:text-[var(--bg-color-active)]">
        <label title="Click to upload" for="{{ $label ?? 'fileImage3'}}" class="flex items-center gap-4 px-6 py-2 cursor-pointer before:border-gray-400/60 hover:before:border-gray-300 group before:bg-gray-100 before:absolute before:inset-0 before:rounded-sm before:border before:border-dashed before:transition-transform before:duration-300 hover:before:scale-100 active:duration-75 active:before:scale-95">
            <div class="relative w-max">
                <i class='bx bx-image-add text-[32px]' ></i>
            </div>
            <div class="relative">
                <span class="relative block text-base font-semibold ">
                    {{  $title ?? 'Upload dopamine 3' }}
                </span>
                <span id="{{ $message ?? 'imageMessage3' }}" class="mt-0.5 block text-sm h-[20px]"></span>
            </div>
            </label>
        <input accept=".json" hidden="" type="file" name="dopamine3" id="{{ $label ?? 'fileImage3'}}" >
    </div>
    <div>
        @error($name ?? 'image')
...
<script>
      ...
      document.addEventListener('DOMContentLoaded', function() {
        const labelId = '{{ $label ?? 'fileImage3' }}';
        const messageId = '{{ $message ?? 'imageMessage3' }}';
